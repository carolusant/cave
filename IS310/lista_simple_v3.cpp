...
//funcion para borrar el primer nodo de la lista
	...
	void borrarNodo(int valor){
			...
			//verificar su el nodo a eliminar es el primero
			if (obtenerNodo(valor)==primero) {
				borrarCabeza();
				return; // salir para que no continue la funcion 
			}

			//verificar su el nodo a eliminar es el ultimo
			if (obtenerNodo(valor)==ultimo) {
				borrarCola();
				return; 
			}
			//si ninguna de las anteriores se ejecuta que continue 
			...
			while(nodoBuscado->datos != valor && nodoBuscado->siguiente!=NULL){ 
			...
			nodoAnterior->siguiente= nodoSiguiente;  // crear el nuevo enlace 
			delete nodoBuscado; 
